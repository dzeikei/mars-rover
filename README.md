...
## Assumptions
- NASA sends commands to the rovers only once
- Rovers will not collide with other rovers in the same grid

## Design decisions
- Number of rovers is defined in the message
- Heading uses cardinal directions instead of angles for simplicity
- Commands that will make the rover fall off the plateau will stop execution
  and execution will skip to the next rover



## Execution
`message.txt` contains the input test string

Run with 
```
./gradlew run --args="message.txt"
```